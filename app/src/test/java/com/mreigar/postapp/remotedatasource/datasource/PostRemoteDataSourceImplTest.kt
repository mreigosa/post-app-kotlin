...
class PostRemoteDataSourceImplTest : AutoCloseKoinTest() {
    ...
    @Test
    fun `given success response, comments are retrieved`() {
        mockServer.enqueueFile("getComments.json")
        val response = PostRemoteDataSourceImpl(postApi).getComments()
    ...
    fun `that cannot fetch  comments`() {
        mockServer.enqueue(500)
        PostRemoteDataSourceImpl(postApi).getComments()
    }
}