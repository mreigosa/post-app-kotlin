...
open class BaseScreenTest : PostAppTestRunner() {
    ...
    fun injectMockModules() {
        val mockModules = module {
            single(override = true) { givenPostRemoteDataSource(configuration = postRemoteDataSourceConfiguration) }
            single(override = true) { givenPostDatabaseDataSource(configuration = postDatabaseDataSourceConfiguration) }
